...
#!/usr/bin/env python3
"""
api.py — FastAPI backend for INT Interview Bot (Multi-Session) v4

KEY CHANGES FROM v3:
─────────────────────────────────────────────────────────────────────
StartRequest gains four new fields:
  interviewer_name, target_role, interview_type, tone

These are passed as env vars to main.py subprocess so that:
1. main.py can call build_greeting_prompt() to get the minimal greeting prompt
2. InterviewerAgent.greet() uses this separate prompt and correctly
   introduces itself by name (fixes "I'm glad to have you here" without name)
...
Endpoints:
  ...
  POST /generate-prompt → build interview prompt (instant, no GPT call)
  GET  /health          → health check
"""
...
class StartRequest(BaseModel):
    ...
    persona: str
    duration_minutes: int = 60
    # ↓ NEW fields — needed so main.py can build the correct greeting prompt
    interviewer_name: str = "Alex"
...
@app.post("/generate-prompt")
async def generate_prompt(req: GeneratePromptRequest):
    """Builds the master prompt. Returns instantly — no GPT call."""
    persona = generate_interview_prompt(
        interviewer_name=req.interviewer_name,
        interview_type=req.interview_type,
        target_role=req.target_role,
    ...
    print(f"[API] 📝 Prompt built for: {req.target_role}", flush=True)
    return {"persona": persona}
...
async def start_bot(req: StartRequest):
    """
    Starts a new isolated bot session.
    Passes interviewer_name, target_role, interview_type, tone as env vars
    so main.py can build the correct greeting prompt for greet().
    """
    session_id = str(uuid.uuid4())
    sid8 = _short_id(session_id)

    chrome_profile = f"/tmp/chrome-profile-{session_id}"
    mic_sink   = f"VMic_{sid8}"
    spk_sink   = f"VSpk_{sid8}"
    mic_source = f"VMicSrc_{sid8}"

    # Add 5 minute buffer so bot doesn't cut off mid-sentence at the deadline
    ...
    # ↓ NEW: passed to main.py for building the greeting prompt
    env["INTERVIEWER_NAME"]      = req.interviewer_name
    env["TARGET_ROLE"]           = req.target_role
    env["INTERVIEW_TYPE"]        = req.interview_type
    env["TONE"]                  = req.tone
    ...
    print(f"[API] ✅ Session started: {sid8} (PID {proc.pid})", flush=True)
    print(f"[API]    Meet     : {req.meetLink}", flush=True)
    print(f"[API]    Duration : {req.duration_minutes} min → stay {stay_seconds}s", flush=True)
    print(f"[API]    Sinks    : {mic_sink} / {spk_sink} / {mic_source}", flush=True)
    print(f"[API]    Profile  : {chrome_profile}", flush=True)
